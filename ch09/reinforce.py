"""REINFORCE policy and return estimator for the chapter 09 lesson."""
...
class Policy(nn.Module):
    """Categorical policy network for a vector observation."""
...
def discounted_returns(rewards: list[float], gamma: float = 0.99) -> torch.Tensor:
    """Build the Monte Carlo return G_t for every time step."""
...
def policy_loss(log_probs: torch.Tensor, returns: torch.Tensor) -> torch.Tensor:
    """Return the negative log-probability objective used by REINFORCE."""
...
def run(
    ...
    *,
    ...
    """Train a local policy on bounded CartPole episodes."""
    ...
    try:
        for episode in range(episodes):
            ...
            loss = policy_loss(torch.stack(log_probs), returns)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        env.close()
    metadata = {
        "experiment": "chapter09_reinforce",
        "seed": seed,
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="chapter09-reinforce-runs")
